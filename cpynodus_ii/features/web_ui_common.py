...
</style>{script_block}</head><body><div class="shell"><aside><div class="brand">NodusWeb<small>{hostname}</small></div><nav>{nav}</nav></aside><main><header><h1>{title}</h1></header><section class="panel">{body}</section></main></div></body></html>""".format(
