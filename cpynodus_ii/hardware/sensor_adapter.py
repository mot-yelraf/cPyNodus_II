...
def bind_sensor_hardware(sensor_runtime, runtime_config, *, board_module=None, busio_module=None):
    ...
    if sensor_runtime.interface == "modbus_rs485" and sensor.modbus is not None:
        tx = _resolve_pin(board_module, sensor.modbus.uart_tx, "missing_modbus_uart_tx_object", errors)
        rx = _resolve_pin(board_module, sensor.modbus.uart_rx, "missing_modbus_uart_rx_object", errors)
        if errors:
            return SensorHardwareAdapter(
                phase="error",
                interface=sensor_runtime.interface,
        ...
        transport = busio_module.UART(
            tx,
            rx,
            baudrate=sensor.modbus.baud,
            timeout=sensor.modbus.timeout_s,
        )
        return SensorHardwareAdapter(
            phase="bound",
            interface=sensor_runtime.interface,
